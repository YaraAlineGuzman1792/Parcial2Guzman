...
void crearArchivos(int cantidadArchivos) {
    ...
    for (int i = 1; i <= cantidadArchivos; i++) {
        // Definir el directorio según si la cantidad de archivos es par o impar
        if (cantidadArchivos % 2 == 0) {
            directorio = "C:\\Users\\alumno\\Documents\\par";
        } else {
            directorio = "C:\\Users\\alumno\\Desktop\\impar";
        }
...
int buscarArchivoMasGrande(string directorio) {
    ...
    cout << "El archivo mas grande es " << nombreArchivoMasGrande << " con " << maxLineas << " lineas." << endl;
    return maxLineas;
}
